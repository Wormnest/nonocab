 ...
 */
 class SubSum 
 {
	...
	static function GetRandomSubSum(reportlist, min_utility) {
		Log.logInfo("Get random sub sub with a minimum utilty of: " + min_utility + " len: " + reportlist.len());
		...
		while (AIController.GetTick() - startTicks < 150) {
			...
			if (best_utility < utility) {
				Log.logInfo("Replace " + best_utility + " with " + utility);
 	...
 	static function GetSubSum(reportList) {
		...
		Log.logInfo("Current income per month: " + incomePerMonth);
		...
		foreach (report in futureBestSubSum[0]) {
			...
			availableMoney -= report.GetCost(availableMoney);
			Log.logDebug(report.ToString());
		}
		Log.logInfo("Future best subsum! " + futureIncomePerMonth);
		...
		Log.logInfo("Break even point: " + breakEven + " months");
		...
		Log.logInfo("Money made till break even: " + moneyMadeTillBreakEven);
		...
		Log.logInfo("Spinoff income per month: " + spinoffIncomePerMonth);
		...
		Log.logInfo("Future spinoff income per month: " + spinoffFutureIncomePerMonth);
		...
		if (spinoffIncomePerMonth + currentIncomePerMonth < futureIncomePerMonth + spinoffFutureIncomePerMonth) {
			...
			Log.logInfo("- Return the future sub sum and return the common reports!");
			...
			Log.logInfo("- Return the current sub sum!");
		}
